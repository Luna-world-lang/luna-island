...
export function initAdminOps(backend, refresh) {
  ...
  $('#ops-board').addEventListener('change',e=>{const row=e.target.closest('[data-player-id]');if(!row||busy||!backend.isAdmin)return;const p=draft.find(p=>p.id===row.dataset.playerId),field=e.target.dataset.edit;try{if(field==='team'){move(p.id,e.target.value);return;}if(field==='cost'){const n=Number(e.target.value);if(e.target.value===''||!Number.isFinite(n)||n<0||n>999)throw Error('코스트는 0~999로 입력하세요.');p.cost=n;}else if(field==='tier'){p.tier=e.target.value;p.cost=tierCost(p.tier);}invalidate();renderBoard();}catch(error){say(error.message);renderBoard();}});
