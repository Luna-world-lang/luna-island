import {balance, tiers, tierCost, parseCSV, aggregate} from './ops-core.js?v=cost-28';
...
export function initAdminOps(backend, refresh) {
  ...
  const rosterButton=document.createElement('button');rosterButton.id='open-team-roster';rosterButton.type='button';rosterButton.className='admin-button';rosterButton.hidden=true;rosterButton.textContent='팀 편성';
  document.querySelector('.site-header').append(rosterButton);
  ...
  <p id="ops-feedback" role="status"></p><div class="ops-tabs"><button data-tab="roster">팀 편성</button><button data-tab="import">결과 가져오기</button><button data-tab="backup">백업 · 복원</button></div>
  <section data-panel="roster"><p>출석·결석은 스트리머 리더보드에서 지정하세요. 여기서는 출석자를 드래그해 팀에 놓거나 카드의 팀 선택을 이용하세요. 변경한 편성은 저장 후 공개 대시보드에 반영됩니다.</p>
  <div class="ops-toolbar"><label>사용할 팀 수<input id="ops-team-count" type="number" min="1" max="8" value="8"></label><label>팀당 정원<input id="ops-capacity" type="number" min="1" max="20" value="3"></label><button id="ops-auto">균형 자동 분배</button><button id="ops-save" class="primary-button">팀 편성 저장</button><button id="ops-reload">저장 상태 다시 읽기</button></div>
  <p class="drawer-help">티어별 기본 코스트는 SSS 8 → SS 7 → S 6 → A 5 → B 4 → C 3 → D 2 → E 1 → 닭 0 → 나뭇가지 -1입니다. 코스트를 직접 조정할 수 있으며, 자동 분배는 인원수와 코스트 합계를 균형 있게 맞춥니다.</p><p id="ops-count"></p><div id="ops-board"></div></section>
  <section data-panel="import" hidden><h3>게임 ID로 결과 불러오기</h3><div class="ops-toolbar"><label>게임 ID<input id="ops-game-id" inputmode="numeric" maxlength="12" placeholder="예: 64894800"></label><button id="ops-game-lookup">경기 조회</button></div><p>게임 ID 조회에는 공식 API 키 등록이 필요합니다. 공식 경기 결과를 조회한 뒤 게임 속 팀과 루나섬 팀을 연결합니다. 등록된 게임 닉네임을 기준으로 팀을 제안하며, 저장 전 직접 확인할 수 있습니다.</p><div id="ops-game-teams"></div><button id="ops-game-apply" hidden>연결한 팀으로 집계 준비</button><hr><h3>또는 결과 파일 가져오기</h3><p>CSV/TSV 파일의 열을 연결해 미리보기한 뒤 저장합니다.</p>
  ...
  async function load(){
    ...
    data=current;revision=ops.revision;backupList=await backend.manageBackup('list');backups=backupList.active;
  ...
  function renderBoard(){
    ...
    const card=p=>`<article class="ops-player" draggable="${p.attendance==='present'}" data-player-id="${esc(p.id)}"><strong>${esc(p.name)}</strong><div class="ops-player-fields"><span>출석 상태: ${p.attendance==='present'?'출석':p.attendance==='absent'?'결석':'미확인'}</span><label>티어<select data-edit="tier">${tiers.map(t=>`<option value="${esc(t)}" ${p.tier===t?'selected':''}>${esc(t||'미등록')}</option>`).join('')}</select></label><label>코스트<input data-edit="cost" type="number" min="-1" max="999" step="0.1" value="${p.cost}"></label><label>팀 선택<select data-edit="team"><option value="">미배정</option>${data.teams.map(t=>`<option value="${esc(t.id)}" ${p.team===t.id?'selected':''}>${esc(t.name)}</option>`).join('')}</select></label></div></article>`;
  ...
  $('#ops-board').addEventListener('change',e=>{const row=e.target.closest('[data-player-id]');if(!row||busy||!backend.isAdmin)return;const p=draft.find(p=>p.id===row.dataset.playerId),field=e.target.dataset.edit;try{if(field==='team'){move(p.id,e.target.value);return;}if(field==='cost'){const n=Number(e.target.value);if(e.target.value===''||!Number.isFinite(n)||n< -1||n>999)throw Error('코스트는 -1~999로 입력하세요.');p.cost=n;}else if(field==='tier'){p.tier=e.target.value;p.cost=tierCost(p.tier);}invalidate();renderBoard();}catch(error){say(error.message);renderBoard();}});
  ...
  $('#ops-save').onclick=()=>action(async()=>{
    ...
    await backend.ops('save',{players:draft.map(({id,tier,cost,attendance,team})=>({id,tier,cost,attendance,team})),capacity,teamCount},revision);await load();await refresh();say('티어·코스트·팀 편성을 저장했습니다.');
  });
